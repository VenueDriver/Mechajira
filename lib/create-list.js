...
CreateList.prototype.processIssues = async function(issueList, issueRecordsKey='issues') {
  ...
  for(var i=0, length = issueRecords.length; i<length; i++) {
    ...
    if(issueRecordsKey == 'sub-tasks') {
      mergedFields.issuetype = 'Sub-task'
    ...
    try {
      ...
      issueFields.key = jiraIssue.issueKey
      if(!this.configData['silent']) {
        console.log(chalk.bold.green(`${jiraIssue.issueKey} created`) +
